...
// Direct UTF-8 codepoint -> phoneme mapping for phoneme_type "text" models.
static void phonemizeTextCodepoints(
    const std::string &text, const PhonemizeConfig &phonemizeConfig,
    std::vector<std::vector<Phoneme>> &result) {
  if (!utf8::is_valid(text.begin(), text.end())) {
    spdlog::error("phonemizeTextCodepoints: invalid UTF-8");
    result.push_back({});
    return;
  }

  std::string processed = una::cases::to_lowercase_utf8(text);
  processed = una::norm::to_nfd_utf8(processed);

  std::vector<Phoneme> sentence;
  auto utext = una::ranges::utf8_view{processed};
  for (auto ch : utext) {
    if (phonemizeConfig.phonemeMap.has_value()) {
      auto mapped = phonemizeConfig.phonemeMap->find(ch);
      if (mapped != phonemizeConfig.phonemeMap->end()) {
        for (const auto &p : mapped->second) {
          sentence.push_back(p);
        }
        continue;
      }
    }
    sentence.push_back(static_cast<Phoneme>(ch));
  }
...
void loadVoice(PiperConfig &config, std::string modelPath,
  ...
  if (voice.configRoot.contains("espeak") && voice.configRoot["espeak"].contains("voice")) {
    voice.espeakVoice = voice.configRoot["espeak"]["voice"].get<std::string>();
  }
  ...
  for (const auto& segment : textSegments) {
    if (segment.isPhonemes) {
      ...
      if (voice.phonemizeConfig.phonemeType == OpenJTalkPhonemes) {
        ...
        std::string espeakVoice = voice.espeakVoice.empty() ? "en" : voice.espeakVoice;
        phonemizeEspeak(segment.text, espeakVoice, segmentPhonemes);
        if (segmentPhonemes.empty() ||
            (segmentPhonemes.size() == 1 && segmentPhonemes[0].empty())) {
          spdlog::error("espeak failed to phonemize text for voice {}", espeakVoice);
        }
      } else if (voice.phonemizeConfig.phonemeType == TextPhonemes) {
        // Split at .!? so text-type models get per-sentence synthesis pauses
        // (same path as espeak/multilingual; fixes one-breath long paragraphs).
        auto sentences = splitTextToSentences(segment.text, TextPhonemes, 0);
        if (sentences.empty()) {
          sentences.push_back(segment.text);
        }
        for (const auto &sentence : sentences) {
          phonemizeTextCodepoints(sentence, voice.phonemizeConfig, segmentPhonemes);
        }
        if (segmentPhonemes.empty()) {
          spdlog::error("text phonemize produced no phonemes for segment");
        }
