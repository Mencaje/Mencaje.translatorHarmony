    ...
    PiperPlusEngine      **out_engine)
{
    ...
    *out_engine = nullptr;
    ...
    try {
        ...
        if (config->espeak_data_dir && config->espeak_data_dir[0] != '\0') {
            engine->config.espeakDataPath = config->espeak_data_dir;
            std::string espeakErr;
            if (!piper::ensureEspeakInitialized(engine->config.espeakDataPath, espeakErr)) {
                set_error(espeakErr);
