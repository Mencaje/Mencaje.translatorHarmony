...
bool SynthesizerTrn::isValid() const
{
    if (priv_ == nullptr) {
        return false;
    }
    const SYN_DATA_t *synData = static_cast<const SYN_DATA_t *>(priv_);
    if (synData->textEncoder_ == nullptr || synData->dec_ == nullptr ||
        synData->flow_ == nullptr || synData->durPredicator_ == nullptr) {
        return false;
    }
    if (synData->langType_ == LANG_TYPE_CHS) {
        return synData->jieba_ != nullptr && synData->hz2ID_ != nullptr;
    }
    if (synData->langType_ == LANG_TYPE_ENG) {
        return synData->eng2Ipa_ != nullptr;
    }
    return false;
}
...
SynthesizerTrn::~SynthesizerTrn()
{
    ...
    SYN_DATA_t *synData = static_cast<SYN_DATA_t *>(priv_);
    priv_ = nullptr;
    if (synData->textEncoder_ != nullptr) {
        delete synData->textEncoder_;
        synData->textEncoder_ = nullptr;
    }
    if (synData->durPredicator_ != nullptr) {
        delete synData->durPredicator_;
        synData->durPredicator_ = nullptr;
    }
    if (synData->flow_ != nullptr) {
        delete synData->flow_;
        synData->flow_ = nullptr;
    }
    if (synData->dec_ != nullptr) {
        delete synData->dec_;
        synData->dec_ = nullptr;
    }
    if (synData->hz2ID_ != nullptr) {
        delete synData->hz2ID_;
        synData->hz2ID_ = nullptr;
    }
    if (synData->tnProcessor_ != nullptr) {
        delete synData->tnProcessor_;
        synData->tnProcessor_ = nullptr;
    }
    if (synData->jieba_ != nullptr) {
        delete synData->jieba_;
        synData->jieba_ = nullptr;
    }
    if (synData->eng2Ipa_ != nullptr) {
        delete synData->eng2Ipa_;
        synData->eng2Ipa_ = nullptr;
    }
    delete synData;
}
